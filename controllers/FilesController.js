...
const fileQ = new Queue('fileQ', 'redis://127.0.0.1:6379');
...
class FilesController {
  ...
  static async postUpload(req, res) {
    ...
    const { name } = req.body;
    const { type } = req.body;
    const { parentId } = req.body.isPublic || false;
    const { data } = req.body;
    if (!name) {
	    return res.status(400).json({ error: 'Missing name' });
    }
    if (!type) {
	    return res.status(400).json({ erro: 'Missing type' });
    }
    if (type !== 'folder' && !data) {
	    return res.status(400).json({ erro: 'Missing data' });
    }
    ...
    if (type === 'folder') {
	    ...
	    const bffer = Buffer.from(data, 'base64');
	    try {
        try {
